  ...
  <header
    ...
    <div class="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
      <div
        ...
        <HeaderLogo />
        <div v-if="showProjectSelect && showMenuToggle">
          ...
          <div class="w-1/2">
            <div v-if="showProjectSelect" class="relative z-20 bg-transparent">
              <select
                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                :class="selectedProject && 'text-gray-800 dark:text-white/90'">
                <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                  @click="selectedProject = ''">
                  Select project
                </option>
                <option v-for="project in projects" :key="project.name" :value="project.name"
                  class="text-gray-700 dark:bg-gray-900 dark:text-gray-400" @click="selectedProject = project.name">
                  {{ project.name }}
                </option>
              </select>
              <span
...
import HeaderLogo from '~/components/layout/dashboard/header/HeaderLogo.vue'
import UserMenu from '~/components/layout/dashboard/header/UserMenu.vue'
...
const projects = ref([
  {
    name: 'Project 1',
    path: '/console/project/1',
  },
  {
    name: 'Project 2',
    path: '/console/project/2',
  },
])
</script>
